...
const rootClass = computed(() => {
  ...
  if (isActive.value) {
    ...
    if ($tabs?.tabProps.value.activeBgColor) classes.append(`bg-color--${$tabs.tabProps.value.activeBgColor}`)
    if ($tabs?.tabProps.value.activeColor) classes.append(`color--${$tabs.tabProps.value.activeColor}`)
    if ($tabs?.tabProps.value.activeClass) classes.append($tabs.tabProps.value.activeClass)
  } else {
    // TODO: заменить на актуальные глобальные классы после их определения
    if ($tabs?.tabProps.value.textColor) classes.append(`color--${$tabs.tabProps.value.textColor}`)
  }
